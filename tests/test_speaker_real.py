...
def smoke(gate_mode: str):
    ...
    logger.info(f"=== gate_mode={gate_mode} === base N={model.config.num_hidden_layers} "
    ...
    with torch.no_grad():
        ...
        logger.info(f"base loss {out_base.loss.item():.4f}")
    ...
    logger.info(f"gated {len(cfg.gated_layers)}, gate params {n_gate}")
    if gate_mode == "moe":
    ...
    logger.info(f"soft loss {out.loss.item():.4f}")
    aux = mod_model.get_aux_loss()
    logger.info(f"aux {aux.item() if aux is not None else None}")
    logger.info(f"counts(hard) {mod_model.get_active_counts()}")
    ...
    with torch.no_grad():
        ...
        logger.info(f"hard loss {out_h.loss.item():.4f} counts {mod_model.get_active_counts()}")
    logger.info(f"Qwen smoke passed ({gate_mode})")
